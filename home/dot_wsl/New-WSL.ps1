...
$cloudInitConfigTmpl = Join-Path "$PSScriptRoot" '.wsl' 'cloud-init' 'Ubuntu-24.04.user-data.yml'
...
if (-not (Test-Path '~\.wsl\cloud-init')) {
    Write-Verbose "Creating cloud-init directory at ~\.wsl\cloud-init"
    New-Item -ItemType Directory -Path '~\.wsl\cloud-init' | Out-Null
}
...
$cloudInitConfigFile = Join-Path '~\.wsl\cloud-init' 'Ubuntu-24.04.user-data'
try {
