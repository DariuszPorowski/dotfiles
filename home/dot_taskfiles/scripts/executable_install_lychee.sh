...
# Constants
readonly GITHUB_OWNER="lycheeverse"
readonly GITHUB_REPO="lychee"
readonly TOOL_NAME="lychee"
...
downloadUrl="$(jq -r --arg arch "${arch}" \
  '.assets[] | select(.browser_download_url | endswith("\($arch)-unknown-linux-gnu.tar.gz")) | .browser_download_url' \
  "${releaseJson}")"
